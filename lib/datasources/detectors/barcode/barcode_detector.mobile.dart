...
class BarcodeDetector implements IBarcodeDetector {
  ...
  int count = 0;
  @override
  Future<Payload?> detect(dynamic detectable, List<BarcodeFormats>? formats,
      ...
      if (detectable?.image is google_barcode.InputImage) {
        ...
        if (formats != null) {
          for (BarcodeFormats format in formats) {
            if ((format == BarcodeFormats.aztec) &&
                (!myFormats.contains(google_barcode.BarcodeFormat.aztec))) {
              myFormats.add(google_barcode.BarcodeFormat.aztec);
            } else if ((format == BarcodeFormats.code39) &&
                (!myFormats.contains(google_barcode.BarcodeFormat.code39))) {
              myFormats.add(google_barcode.BarcodeFormat.code39);
            } else if ((format == BarcodeFormats.code93) &&
                (!myFormats.contains(google_barcode.BarcodeFormat.code93))) {
              myFormats.add(google_barcode.BarcodeFormat.code93);
            } else if ((format == BarcodeFormats.codabar) &&
                (!myFormats.contains(google_barcode.BarcodeFormat.codabar))) {
              myFormats.add(google_barcode.BarcodeFormat.codabar);
            } else if ((format == BarcodeFormats.code128) &&
                (!myFormats.contains(google_barcode.BarcodeFormat.code128))) {
              myFormats.add(google_barcode.BarcodeFormat.code128);
            } else if ((format == BarcodeFormats.datamatrix) &&
                (!myFormats
                    .contains(google_barcode.BarcodeFormat.dataMatrix))) {
              myFormats.add(google_barcode.BarcodeFormat.dataMatrix);
            } else if ((format == BarcodeFormats.ean8) &&
                (!myFormats.contains(google_barcode.BarcodeFormat.ean8))) {
              myFormats.add(google_barcode.BarcodeFormat.ean8);
            } else if ((format == BarcodeFormats.ean13) &&
                (!myFormats.contains(google_barcode.BarcodeFormat.ean13))) {
              myFormats.add(google_barcode.BarcodeFormat.ean13);
            } else if ((format == BarcodeFormats.itf) &&
                (!myFormats.contains(google_barcode.BarcodeFormat.itf))) {
              myFormats.add(google_barcode.BarcodeFormat.itf);
            } else if ((format == BarcodeFormats.pdf417) &&
                (!myFormats.contains(google_barcode.BarcodeFormat.pdf417))) {
              myFormats.add(google_barcode.BarcodeFormat.pdf417);
            } else if ((format == BarcodeFormats.qrcode) &&
                (!myFormats.contains(google_barcode.BarcodeFormat.qrCode))) {
              myFormats.add(google_barcode.BarcodeFormat.qrCode);
            } else if ((format == BarcodeFormats.upca) &&
                (!myFormats.contains(google_barcode.BarcodeFormat.upca))) {
              myFormats.add(google_barcode.BarcodeFormat.upca);
            } else if ((format == BarcodeFormats.upce) &&
                (!myFormats.contains(google_barcode.BarcodeFormat.upce))) {
