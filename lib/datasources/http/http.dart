...
class Http {
  ...
  static Future<HttpResponse> post(String url, String body,
    ...
    try {
      ...
      if (uri != null) {
        // execute request
        Response response = await dart_http
            .post(uri, headers: encodeHeaders(headers), body: body)
            .timeout(Duration(
                seconds: (((timeout != null) && (timeout > 0))
                    ? timeout
                    : defaultTimeout)));
  ...
  static Future<HttpResponse> patch(String url, String? body,
      {Map<String, String>? headers, int? timeout = 60}) async {
    try {
      // convert url
      Uri? uri = encodeUri(url, refresh: false);
      if (uri != null) {
        // execute request
        Response response = await dart_http
            .patch(uri, headers: encodeHeaders(headers), body: body)
            .timeout(Duration(
                seconds: (((timeout != null) && (timeout > 0))
                    ? timeout
                    : defaultTimeout)));
  ...
  static Future<HttpResponse> delete(String url,
    ...
    try {
      ...
      if (uri != null) {
        // execute request
        Response response = await dart_http
            .delete(uri, headers: encodeHeaders(headers))
            .timeout(Duration(
                seconds: (((timeout != null) && (timeout > 0))
                    ? timeout
                    : defaultTimeout)));
        ...
        return HttpResponse(url,
            statusCode: HttpStatus.internalServerError,
            statusMessage: "Url $url is null or invalid");
      }
    } catch (e) {
      return HttpResponse(url,
          statusCode: HttpStatus.internalServerError,
