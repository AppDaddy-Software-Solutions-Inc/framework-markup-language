...
class DialogManager {
    ...
    if (request.type == DialogType.modal) {
      ...
      if (request.title != null && request.title != '') {
        closeButton =
          ...
          Expanded(
              child:
                  Text(request.title!, style: const TextStyle(fontSize: 16))),
          closeButton,
        ]);
      }
      ...
      AlertDialog dialog = AlertDialog(
          ...
          insetPadding: EdgeInsets.symmetric(
              horizontal: FmlEngine.isMobile ? 10 : 128,
              vertical: FmlEngine.isMobile ? 64 : 128),
          titlePadding: const EdgeInsets.all(0),
          content: request.content,
          contentPadding: EdgeInsets.only(
              top: 0,
              bottom: 10,
              left: FmlEngine.isMobile ? 2 : 10,
              right: FmlEngine.isMobile ? 2 : 10));
      ...
      Alert dialog = Alert(
          ...
          buttons: buttons,
          closeFunction: () {
            completer.complete(AlertResponse(pressed: -1));
            Navigator.of(context).pop();
          });
  ...
  static Future<AlertResponse> _show(
