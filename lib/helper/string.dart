...
class S
{
  ...
  static String parseEmojis(String val) {
    String value = val;
    try {
      value = value.replaceAllMapped(RegExp(r'(?=:)([^\s])(.*?)([^\s])(:)'), (Match m) {
        String? m2;
        if ((m[0]?.startsWith(':') ?? false) && (m[0]?.endsWith(':') ?? false) && !(m[0]?.contains(' ') ?? false)) m2 = (m[0]!.substring(1, m[0]!.length-1));
        if (!S.isNullOrEmpty(m2) && !(m2?.contains(':') ?? false) && !(m2?.contains(' ') ?? false)  && Emoji.emoji[m2?.toLowerCase()] != null) {
  ...
  static bool isBool(dynamic b)
  {
    try
    {
      ...
      return ((b == 'false') || (b == '0') || (b == 'no') || (b == 'true') || (b == '1') || (b == 'yes'));
    }
    catch(e)
    {
      return false;
  ...
  static UriData? toDataUri(dynamic uri)
  {
    try
    {
      ...
      if ((uri is String) && (uri.toLowerCase().trim().startsWith("data:"))) return UriData.parse(uri);
      return null;
    }
    catch(e)
    {
  ...
  {
    bool valid = RegExp(r"^[a-zA-Z0-9.!#$%&'*+/=?^_`{|}~-]+@[a-zA-Z0-9](?:[a-zA-Z0-9-]{0,253}[a-zA-Z0-9])?(?:\.[a-zA-Z0-9](?:[a-zA-Z0-9-]{0,253}[a-zA-Z0-9])?)*$").hasMatch(value);
    return valid;
  }
...
//    if (!hasNumber(value)) return Phrase.passwordNoNumber;
  ...
  static Color? toColor(String? color)
  {
    ...
    {
      c = null;
      Log().debug(e.toString(), caller: 'helper/string.dart => Color toColor(String color)');
    }
    return c;
  }
  ...
  static DateTime? toDate(String? datetime, {String? format})
  {
    ...
    {
      result = null;
      Log().debug(e.toString(), caller: 'helper/string.dart => DateTime toDate(String date, String format) => DateFormat(format).parse(date)');
    }
    ...
    {
      result = null;
      Log().debug(e.toString(), caller: 'helper/string.dart => DateTime toDate(String date, String format) => DateTime.tryParse(date)');
    }
    return result;
  }
  ...
  static String? toChar(DateTime? datetime, {String? format})
  {
    ...
    {
      result = null;
      Log().debug(e.toString(), caller: 'helper/string.dart => String toChar(String date, String format)');
    }
    return result;
  }
  ...
  static dynamic mapVal(Map? map, dynamic key, {dynamic defaultValue})
  {
    ...
    {
      Log().debug(e.toString(), caller: 'helper/string.dart => mapVal(Map map, dynamic key, {dynamic defaultValue})');
      return defaultValue;
    }
    return value;
  }
 ...
 static String removeNonASCII(String str)
  ...
  static String newId({String prefix = "auto"}) => "$prefix${Uuid().v4().replaceAll("-", "").toLowerCase()}";
}
