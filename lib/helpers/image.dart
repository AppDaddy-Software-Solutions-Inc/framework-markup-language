...
class ImageHelper {
  ...
  static int getLuminanceRgb(int red, int green, int blue) =>
      (0.299 * red + 0.587 * green + 0.114 * blue).round();
  ...
  static List<int> toGrayscalePixelsFromRgba(List<int> rgba) {
    return List<int>.generate(rgba.length ~/ 4,
        (index) => ImageHelper.getColorFromByte(rgba, index * 4));
  }
  ...
  static int byte2Pixel(Uint8List byteData, int firstIndex) {
    return byteData[firstIndex + 3] <<
        24 + byteData[firstIndex] <<
        16 + byteData[firstIndex + 1] <<
        8 + byteData[firstIndex + 2];
  }
