...
class TemplateManager {
  ...
  FetchResult _fromMemory(Uri uri) {
    if (templates.containsKey(uri.url)) {
      return FetchResult(document: templates[uri.url]);
    }
    return FetchResult(
        code: HttpStatus.notFound,
        message: Phrases().pageNotFound,
        detail: uri.url);
  }
  ...
  Future<FetchResult> _fromDatabase(String key) async {
    try {
      ...
      return form != null
          ? FetchResult(template: form.template)
          : FetchResult(
              code: HttpStatus.notFound,
              message: Phrases().formNotFound,
              detail: key);
    } catch (e) {
      Log().exception(e);
      return FetchResult(
          code: HttpStatus.internalServerError,
          message: "Error getting template from the Database",
          detail: e.toString());
    }
  }
  ...
  Future<FetchResult> _fromAssetsBundle(String url) async {
    try {
      ...
      var template = await rootBundle
          .loadString(url.replaceFirst("file://", "assets/"), cache: false);
  ...
  Future<FetchResult> _fromDisk(String url) async {
    try {
      ...
      return template != null
          ? FetchResult(template: template)
          : FetchResult(
              code: HttpStatus.notFound,
              message: Phrases().fileNotFound,
              detail: url);
    } catch (e) {
      Log().exception(e);
      return FetchResult(
  ...
  Future<FetchResult> _fromServer(String url) async {
    ...
    if (System().connected) {
      ...
      try {
        ...
        response = HttpResponse(url,
            statusCode: HttpStatus.badGateway, statusMessage: e.toString());
      }
      ...
      // error
      Log().error(
          "Error fetching template from $url. Error is ${response.statusCode} ${response.statusMessage}");

      var msg = Phrases().siteUnreachable;
      if (response.statusCode == HttpStatus.notFound) {
        msg = Phrases().pageNotFound;
      }

      return FetchResult(
          code: response.statusCode,
          message: "${response.statusCode} $msg",
          detail: url);
    }
    ...
    return FetchResult(
        ...
        message: Phrases().notConnected,
        detail: Phrases().checkConnection);
  }
  ...
  Future<Template> _fetchFromDatabase(
    ...
    return Template.fromXmlDocument(
        name: url, xml: result.document, parameters: parameters);
  }

  Future<Template> _fetchFromHtml(
      String url, Map<String, String?> parameters) async {
  ...
  Future<Template> _fetchFromFile(
      String url, Map<String, String?>? parameters) async {
  ...
  Future<Template> _fetchFromServer(String url, Uri uri,
    ...
    bool cache = System.currentApp?.cacheContent ?? false;

    var result = FetchResult();

    // auto refresh?
    refresh = refresh || (System.currentApp?.refresh ?? false);
    ...
    if (result.isFail) {
      ...
      // found in memory? - return the template
      if (result.isSuccess) {
        return Template.fromXmlDocument(
            name: url,
            xml: result.document,
  ...
  Future<String?> _fetchErrorPageFromServer(
      FetchResult result, String back) async {
    if (System.currentApp?.errorPage == null) return null;

    var url = System.currentApp!.errorPage!;
  ...
  FetchResult _parse(String? template) {
    if (template == null) {
      return FetchResult(
          ...
          message: Phrases().errorParsingTemplate);
    }
    try {
  ...
  Future<bool> _processIncludes(XmlDocument document,
    ...
    for (XmlElement element in includes) {
      ...
      if (!exclude) {
        ...
        String url = Binding.applyMap(
            Xml.get(node: element, tag: 'url'), parameters,
            caseSensitive: false)!;
        ...
        if (uri != null) {
          ...
          // fetch the template
          var template =
              await fetch(url: url, parameters: parameters, refresh: refresh);
