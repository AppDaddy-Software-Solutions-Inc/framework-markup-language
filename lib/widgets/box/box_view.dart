...
class _BoxViewState extends WidgetState<BoxView> {
  ...
  BorderRadius? _getRadius(Border? border) {
    ...
    try {
      ...
      if (cornersFromTopRightClockwise.length == 1) {
        radii = List<double?>.filled(
            4, S.toDouble(cornersFromTopRightClockwise[0]));
      } else {
    ...
    BorderRadius? containerRadius = BorderRadius.only(
        topRight:
            Radius.circular(radii.asMap().containsKey(0) ? radii[0] ?? 0 : 0),
        bottomRight:
            Radius.circular(radii.asMap().containsKey(1) ? radii[1] ?? 0 : 0),
        bottomLeft:
            Radius.circular(radii.asMap().containsKey(2) ? radii[2] ?? 0 : 0),
        topLeft:
            Radius.circular(radii.asMap().containsKey(3) ? radii[3] ?? 0 : 0));
  ...
  Widget builder(BuildContext context, BoxConstraints constraints) {
    ...
    BoxDecoration? borderDecoration = border != null
