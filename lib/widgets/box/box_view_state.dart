...
abstract class BoxViewWidgetState<T extends StatefulWidget> extends ViewableWidgetState<T>
  ...
  Widget _buildInnerContent(BoxModel model, WidgetAlignment alignment, List<Widget> children) {
  ...
  // applies padding around the of the box
  Widget _addPadding(BoxModel model, Widget view) {
    if (model.paddingTop != null) {
      var inset = EdgeInsets.only(
          top: model.paddingTop ?? 0,
          right: model.paddingRight ?? 0,
  ...
  Widget buildBox(BoxConstraints constraints, BoxModel model, {List<Widget>? children}) {

    // Check if widget is visible before wasting resources on building it
    if (!model.visible) return const Offstage();
    ...
    // inflate the child list
    children ??= model.inflate();

    // add child's parentData - BoxLayout
    // for flex, wrap, stack, etc.
    children = _wrapParentData(children);
    ...
    // apply constraints
    view = applyConstraints(view, model.constraints);
