...
class _GridViewState extends WidgetState<GridView>
{
  ...
  Widget builder(BuildContext context, BoxConstraints constraints)
  {
    ...
    gridWidth  = widget.model.getSystemMaxWidth()  ?? widget.model.width  ?? MediaQuery.of(context).size.width;
    gridHeight = widget.model.getSystemMaxHeight() ?? widget.model.height ?? MediaQuery.of(context).size.height;

    if (widget.model.items.isNotEmpty) 
    {
      prototypeWidth  = widget.model.items.entries.first.value.width ?? (widget.model.getSystemMaxWidth()   ?? widget.model.width  ?? widget.model.itemSize?.width ?? MediaQuery.of(context).size.width) / (sqrt(widget.model.items.length) + 1);
      prototypeHeight = widget.model.items.entries.first.value.height ?? (widget.model.getSystemMaxHeight() ?? widget.model.height ?? widget.model.itemSize?.height ?? MediaQuery.of(context).size.height) / (sqrt(widget.model.items.length) + 1);
    }
    else 
    {
      prototypeWidth  = (widget.model.getSystemMaxWidth()  ?? widget.model.width  ?? widget.model.itemSize?.width  ?? MediaQuery.of(context).size.width) / (sqrt(widget.model.items.length) + 1);
      prototypeHeight = (widget.model.getSystemMaxHeight() ?? widget.model.height ?? widget.model.itemSize?.height ?? MediaQuery.of(context).size.height) / (sqrt(widget.model.items.length) + 1);
    }
    ...
    if (constraints.maxHeight == double.infinity || constraints.maxHeight == double.negativeInfinity || h == null)
      h = widget.model.getSystemMaxHeight() ?? constraints.maxHeight;
    if (constraints.maxWidth  == double.infinity || constraints.maxWidth  == double.negativeInfinity || w  == null)
      w  = widget.model.getSystemMaxWidth() ?? constraints.maxWidth;
