...
class PluginFieldView extends StatefulWidget implements ViewableWidgetView {

  @override
  final PluginFieldModel model;

  PluginFieldView(this.model) : super(key: ObjectKey(model));

  @override
  State<PluginFieldView> createState() => PluginWidgetViewState();
}

class PluginWidgetViewState extends ViewableWidgetState<PluginFieldView> {
  ...
  $Value? onChange(Runtime runtime, $Value? target, List<$Value?> args)
  {
    int i = 0;
    widget.model.value = args.first;
    return $String("done");
  }
  ...
  Widget build(BuildContext context) {
    try
    {
      ...
      // build view
      var view = runtime!.executeLib(widget.model.library, widget.model.method, [$Closure(onChange)]);
