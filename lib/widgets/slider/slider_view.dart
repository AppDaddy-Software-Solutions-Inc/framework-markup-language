  ...
  Widget build(BuildContext context) {
    ...
    if (max < min) {
      ...
      if (min > max) {
        min = 0;
        max = 0;
        Log().debug('Slider min > max${widget.model.id}',
            caller: '${widget.model.elementName}.$runtimeType');
      }
      if (value1 < min || value1 > max) {
        value1 = min;
        Log().debug('Slider value out of range${widget.model.id}',
            caller: '${widget.model.elementName}.$runtimeType');
      }
    }
    ...
    if (widget.model.range) {
      view = RangeSlider(
          values: RangeValues(toDouble(value1)!, toDouble(value2)!),
          min: min,
          max: max,
          divisions: !isNullOrEmpty(widget.model.divisions) &&
              ...
              : null,
          labels: RangeLabels(value1.toString(), value2.toString()),
          onChanged: (RangeValues values) => onRangeChange(values),
          activeColor: ColorHelper.lighten(
      ...
      view = Slider(
        ...
        onChanged: onChange,
        activeColor: ColorHelper.lighten(
            widget.model.color ?? Theme.of(context).colorScheme.primary, 0.05),
        inactiveColor: Theme.of(context).colorScheme.secondaryContainer,
