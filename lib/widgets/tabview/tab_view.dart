...
class _TabViewState extends WidgetState<TabView> with TickerProviderStateMixin {
  ...
  didChangeDependencies() {
    ...
    EventManager.of(widget.model)
        ?.registerEventListener(EventTypes.trigger, onTrigger, priority: 0);
  ...
  void didUpdateWidget(TabView oldWidget) {
    ...
    if ((oldWidget.model != widget.model)) {
      ...
      EventManager.of(oldWidget.model)
          ?.removeEventListener(EventTypes.trigger, onTrigger);
  ...
  Widget _buildTabsButton({double? height}) {
    ...
    widget.model.views.forEach((url, view) {
      ...
      PopupMenuItem button =
          PopupMenuItem(child: Text(title, style: style), value: i);
